 ...
 *
 * On Linux the preferred location is `$XDG_RUNTIME_DIR/securevault`, a tmpfs owned by the user and
 * cleared at logout, so a copy does not survive a reboot even if cleanup fails. **Windows has no
 * tmpfs equivalent**, so there the copy is written to `%LOCALAPPDATA%\SecureVault\cache` -- on
 * disk, restricted to the user by ACL, and removed on lock, at startup and before each open.
 * [volatileStorage] reports which situation applies so the UI can say so rather than implying the
